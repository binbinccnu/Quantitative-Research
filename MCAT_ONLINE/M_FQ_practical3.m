...
%%
% [r1,r2,r3,r4] = r_adjust(targets1,targets2,targets3,targets4,r1,r2,r3,r4);
% 
%%
for examinee = 1:N
...
for j = 1:length(index_cali)
    ...
    [a1_OEM_FQ_practical(j),a2_OEM_FQ_practical(j),b_OEM_FQ_practical(j)] = OEM_MCAT2D(sparse,examinee_record,test_operational,j,a,b,person_select);
end
[a1_MEM_FQ_practical,a2_MEM_FQ_practical,b_MEM_FQ_practical] = MEM_MCAT2D(sparse,examinee_record,test_operational,test_cali,a,b,index_cali);                             
...
bias_a1_OEM_FQ_practical = sum(abs(a1_OEM_FQ_practical - a(index_cali,1))) / length(index_cali);
bias_a2_OEM_FQ_practical = sum(abs(a2_OEM_FQ_practical - a(index_cali,2))) / length(index_cali);
bias_b_OEM_FQ_practical  = sum(abs(b_OEM_FQ_practical  - b(index_cali)))   / length(index_cali);
rmse_a1_OEM_FQ_practical = sqrt(sum((a1_OEM_FQ_practical - a(index_cali,1)).^2) / length(index_cali));
rmse_a2_OEM_FQ_practical = sqrt(sum((a2_OEM_FQ_practical - a(index_cali,2)).^2) / length(index_cali));
rmse_b_OEM_FQ_practical  = sqrt(sum((b_OEM_FQ_practical  - b(index_cali)).^2)   / length(index_cali));     

bias_a1_MEM_FQ_practical = sum(abs(a1_MEM_FQ_practical - a(index_cali,1))) / length(index_cali);
bias_a2_MEM_FQ_practical = sum(abs(a2_MEM_FQ_practical - a(index_cali,2))) / length(index_cali);
bias_b_MEM_FQ_practical  = sum(abs(b_MEM_FQ_practical  - b(index_cali)))   / length(index_cali);
rmse_a1_MEM_FQ_practical = sqrt(sum((a1_MEM_FQ_practical - a(index_cali,1)).^2) / length(index_cali));
rmse_a2_MEM_FQ_practical = sqrt(sum((a2_MEM_FQ_practical - a(index_cali,2)).^2) / length(index_cali));
rmse_b_MEM_FQ_practical  = sqrt(sum((b_MEM_FQ_practical  - b(index_cali)).^2)   / length(index_cali));     
