...
for examinee = 1:N
    ...
    if zone == 1
        targets = targets1;
        dif = sum(abs(repmat(theta(examinee,:),J,1) - targets1),2);
    elseif zone == 2
        targets = targets2;
        dif = sum(abs(repmat(theta(examinee,:),J,1) - targets2),2);
    elseif zone == 3
        targets = targets4;
        dif = sum(abs(repmat(theta(examinee,:),J,1) - targets4),2);
    elseif zone == 4
        targets = targets3;
        dif = sum(abs(repmat(theta(examinee,:),J,1) - targets3),2);
    end
    ...
    for item = 1:length(item_chosen)
        threshold = threshold_initial * .95 / mvnpdf(targets(item_chosen(item),:),[0 0],[1 0; 0 1]);
%         threshold = threshold_initial * 150 * (target_sample/4 - item_record_zone(item_chosen(item),zone))/ target_sample / mvnpdf(targets(item_chosen(item),:),[0 0],[1 0; 0 1]);
        if sorted_diff(item) <= threshold
