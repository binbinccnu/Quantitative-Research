...
%     b = zeros(n,1);
    ...
    %% divide examinees into four quadrant
    zone = zeros(N,1);
    for examinee = 1:N
        zone(examinee) = zone_divide(theta(examinee,:)); 
    end
    sample = 1:N;
    sample1 = sample(zone == 1);
    sample2 = sample(zone == 2);
    sample3 = sample(zone == 3);
    sample4 = sample(zone == 4);
    ...
    for cali_item_index = 1:length(index_cali)
        ...
        unselect = setdiff(sample1,person_finish); unselect_theta = theta(unselect,:);
        dif1 = abs(a(cali_item,1).*unselect_theta(:,1) + a(cali_item,2).*unselect_theta(:,2) + b(cali_item)) ./ sqrt(a(cali_item,1).^2 + a(cali_item,2).^2);
        dif1 = abs(a(cali_item,1).*unselect_theta(:,1) + a(cali_item,2).*unselect_theta(:,2) ) ./ sqrt(a(cali_item,1).^2 + a(cali_item,2).^2);
        [sort_Dif,sort_index] = sort(dif1);
        person1 = unselect(sort_index(1:target_sample/4));
        
        unselect = setdiff(sample2,person_finish);
        unselect_theta = theta(unselect,:);
        dif2 = abs(a(cali_item,1).*unselect_theta(:,1) + a(cali_item,2).*unselect_theta(:,2) + b(cali_item)) ./ sqrt(a(cali_item,1).^2 + a(cali_item,2).^2);
        dif2 = abs(a(cali_item,1).*unselect_theta(:,1) + a(cali_item,2).*unselect_theta(:,2) ) ./ sqrt(a(cali_item,1).^2 + a(cali_item,2).^2);
        [sort_Dif,sort_index] = sort(dif2);
        person2 = unselect(sort_index(1:target_sample/4));
        
        unselect = setdiff(sample3,person_finish);
        unselect_theta = theta(unselect,:);
        dif3 = abs(a(cali_item,1).*unselect_theta(:,1) + a(cali_item,2).*unselect_theta(:,2) + b(cali_item)) ./ sqrt(a(cali_item,1).^2 + a(cali_item,2).^2);
        dif3 = abs(a(cali_item,1).*unselect_theta(:,1) + a(cali_item,2).*unselect_theta(:,2) ) ./ sqrt(a(cali_item,1).^2 + a(cali_item,2).^2);
        [sort_Dif,sort_index] = sort(dif3);
        person3 = unselect(sort_index(1:target_sample/4));
        
        unselect = setdiff(sample4,person_finish);
        unselect_theta = theta(unselect,:);
        dif4 = abs(a(cali_item,1).*unselect_theta(:,1) + a(cali_item,2).*unselect_theta(:,2) + b(cali_item)) ./ sqrt(a(cali_item,1).^2 + a(cali_item,2).^2);
        dif4 = abs(a(cali_item,1).*unselect_theta(:,1) + a(cali_item,2).*unselect_theta(:,2) ) ./ sqrt(a(cali_item,1).^2 + a(cali_item,2).^2);
