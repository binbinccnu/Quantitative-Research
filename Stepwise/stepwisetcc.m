function [Inew]=stepwisetcc(I,parameter1,parameter2,D,csem)

%% This procedure is 'stepwised' tcc selection of linking items
...
%%  Find potential item that could be entered
if  ~isempty(R)
    %  Find out one item in the remaining set with samllest area difference
    for m=1:length(R)
        diff(m)=areadiff(parameter1([I R(m)],:),parameter2([I R(m)],:),D);
    end
    [~,index]=sort(diff);
    proposal_item_no=R(index(1));

    %  Update I 
    Ipro=[I proposal_item_no];
    
    %  Compute TCC difference with new entered item
    tccdiff1=diff(index(1));
    if (tccdiff1-tccdiff0<=csem)
        I=Ipro;
        tccdiff0=tccdiff1;  
    end
end   
