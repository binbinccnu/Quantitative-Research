...
%     b_hat_sh_sib=meanscore_hat./variance_hat.^(1/2);
%     %gathering the results
%     flag_sib_sh=abs(b_hat_sh_sib)>1.96;
%     
%     flag_sib_sh=find(flag_sib_sh==1);
%     typeIrate_sh_sib=length(flag_sib_sh(flag_sib_sh>round(n*p)))/(n-round(n*p));
%     power_sh_sib=length(flag_sib_sh(flag_sib_sh<=round(n*p)))/round(n*p);
% end
% 
