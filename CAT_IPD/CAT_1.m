sparse_matrix_cat_1=U_1;
examinee_record_CAT_1=zeros(N,test_length);
theta_estimate_cat_1=zeros(N,1);
%
...
for j6=1:N
    ...
    examinee_record_CAT_1(j6,1:m)=examinee_record;
    ...
    for j7=m:test_length
        if j7~=test_length
            ...
            examinee_record_CAT_1(j6,length(examinee_record))=next_item;
    ...
    sparse_matrix_cat_1(j6,setdiff(1:n,examinee_record))=9;
    theta_estimate_cat_1(j6)=theta_hat;
end

rmse_cat_1=sqrt(sum((theta_1-theta_estimate_cat_1).*(theta_1-theta_estimate_cat_1))/N);
%get the exposure rate for each item 
for jj=1:n
    exposure_cat_1(y,jj)=sum(sparse_matrix_cat_1(:,jj)~=9)/N;
end