...
sparse_matrix_sh_1=U_1;
examinee_record_sh_1=zeros(N,test_length);
theta_sh_1=zeros(N,1);
%
'CAT_SH_1'
for j6=1:N
    ...
    examinee_record_sh_1(j6,1:m)=examinee_record;
    ...
    for j7=m:test_length
        if j7~=test_length
            ...
            examinee_record_sh_1(j6,length(examinee_record))=next_item;
    ...
    theta_sh_1(j6)=theta_hat; 
    
    %get the sparse matrix
    sparse_matrix_sh_1(j6,setdiff(1:n,examinee_record))=9;
end

%compute the item exposure rate P(S), namely, S
S=zeros(n,1);
for jj=1:n
    S(jj)=sum(sparse_matrix_sh_1(:,jj)~=9)/N;
end
A_S=ones(n,1);
A=S;
...
while diff>0 && outiter<10
    ...
    for jj=1:n
        A(jj)=sum(sparse_matrix_sh_1(:,jj)~=9)/N;
    end
    diff=sum(abs(A_-A));
...
figure(1)
plot(1:n,A)
title 'Item Exposure Rate for SH CAT, year 1';
